...
def _resolve_output_paths(args):
    if args.output_dir:
        ...
        if args.config_resolved_path is None:
            args.config_resolved_path = os.path.join(
                args.output_dir, 'config_resolved.json')
        if args.final_metrics_path is None:
            args.final_metrics_path = os.path.join(
                args.output_dir, 'final_metrics.json')
...
def run(args):
    ...
    agent_config = {'model_lr': args.model_lr, 'momentum': args.momentum, 'model_weight_decay': args.model_weight_decay,
                    ...
                    'use_cflat': args.use_cflat,
                    'cflat_rho': args.cflat_rho,
                    'cflat_lambda': args.cflat_lambda,
                    'cflat_eps': args.cflat_eps,
                    'cflat_adaptive': args.cflat_adaptive,
                    'cflat_bn_mode': args.cflat_bn_mode,
                    'cflat_on_total_loss': args.cflat_on_total_loss,
                    'cflat_debug': args.cflat_debug,
                    'use_pls_cflat': args.use_pls_cflat,
                    'pls_cflat_mode': args.pls_cflat_mode,
                    'cflat_target_scope': args.cflat_target_scope,
                    'project_before_perturb': args.project_before_perturb,
                    'project_after_aggregate': args.project_after_aggregate,
                    'deep_layer_rule': args.deep_layer_rule,
                    'pls_cflat_debug': args.pls_cflat_debug,
...
def get_args(argv):
    ...
    parser.add_argument('--use_cflat', action='store_true',
                        help="Enable C-Flat gradient generation before Adam-NSCL constraints")
    parser.add_argument('--cflat_rho', type=float, default=0.02,
                        help="C-Flat neighborhood radius")
    parser.add_argument('--cflat_lambda', type=float, default=0.02,
                        help="C-Flat first-order flatness weight")
    parser.add_argument('--cflat_eps', type=float, default=1e-12,
                        help="Numerical stability epsilon for C-Flat")
    parser.add_argument('--cflat_adaptive', action='store_true',
                        help="Enable the adaptive C-Flat perturbation variant")
    parser.add_argument('--cflat_bn_mode', type=str, default='disable_running_stats',
                        choices=['disable_running_stats', 'none'],
                        help="BatchNorm handling for perturbed forwards")
    parser.add_argument('--cflat_on_total_loss', action='store_true',
                        help="Use C-Flat on the full loss including Adam-NSCL regularization")
    parser.add_argument('--cflat_debug', action='store_true',
                        help="Print per-step C-Flat gradient and perturbation norms")
    parser.add_argument('--use_pls_cflat', action='store_true',
                        help="Enable Projected / Layer-Selective C-Flat")
    parser.add_argument('--pls_cflat_mode', type=str, default='projected_layer_selective',
                        choices=['projected_all', 'layer_selective', 'projected_layer_selective'],
                        help="PLS-CFlat mode")
    parser.add_argument('--cflat_target_scope', type=str, default='deep_plus_classifier',
                        choices=['all', 'classifier', 'deep', 'deep_plus_classifier'],
                        help="Parameter scope that participates in C-Flat")
    parser.add_argument('--project_before_perturb', action='store_true',
                        help="Project target gradients before constructing C-Flat perturbations")
    parser.add_argument('--project_after_aggregate', action='store_true',
                        help="Project the aggregated target gradients before optimizer.step()")
    parser.add_argument('--deep_layer_rule', type=str, default='last_stage',
                        choices=['last_block', 'last_stage', 'last_third'],
                        help="Rule for selecting deep layers in layer-selective PLS-CFlat")
    parser.add_argument('--pls_cflat_debug', action='store_true',
                        help="Print scope and projected-gradient diagnostics for PLS-CFlat")
    parser.add_argument('--seed', type=int, default=0,
                        help="Random seed")
    parser.add_argument('--max_tasks', type=int, default=0,
                        help="Run only the first N tasks when > 0")
    parser.add_argument('--max_train_batches', type=int, default=0,
                        help="Limit train batches per epoch when > 0")
    parser.add_argument('--max_eval_batches', type=int, default=0,
                        help="Limit eval batches when > 0")
    parser.add_argument('--output_dir', type=str, default=None,
                        help="Optional directory for resolved config and final metrics")
    parser.add_argument('--config_resolved_path', type=str, default=None,
                        help="Optional JSON path to save the resolved config")
    parser.add_argument('--final_metrics_path', type=str, default=None,
                        help="Optional JSON path to save final metrics")
...
if __name__ == '__main__':
    ...
    _save_json(args.config_resolved_path, vars(args))
    avg_final_acc = np.zeros(args.repeat)
    final_bwt = np.zeros(args.repeat)
    if args.gpuid[0] >= 0 and torch.cuda.is_available():
    ...
    for r in range(args.repeat):
        ...
        repeat_metrics.append({
            'repeat_index': r,
            'repeat_seed': current_seed,
            'avg_acc_history': avg_acc_history,
            'bwt_history': bwt_history,
            'final_avg_acc': avg_final_acc[r],
            'final_bwt': final_bwt[r],
            'wall_clock_seconds': time.time() - repeat_start,
            'task_names': task_names,
            'acc_table': _ordered_to_dict(acc_table),
        })

    _save_json(args.final_metrics_path, {
        'repeat_metrics': repeat_metrics,
        'avg_final_acc': avg_final_acc.tolist(),
        'final_bwt': final_bwt.tolist(),
        'acc_mean': float(avg_final_acc.mean()),
        'acc_std': float(avg_final_acc.std()),
        'bwt_mean': float(final_bwt.mean()),
        'bwt_std': float(final_bwt.std()),
        'seed': args.seed,
        'use_cflat': args.use_cflat,
